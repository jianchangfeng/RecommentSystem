...
#########################################################################
# Author: @appbk.com
# Created Time: Fri 14 Jul 2017 12:00:24 PM CST
# File Name: user_based_offline.py
# Description: 
######################################################################
import json
import time
...
def get_recommend(action_data, uid, sim=cos_distance):
    ...
    for other_uid in action_data:
        ...
        for vid in other_action:
            if not uid_action.has_key(vid):#用户已经看过的视频不计算
    ...
    final_recommend_list = sorted(recommend_list, key=lambda d:d[1], reverse=True)
...
def get_all_recommend(filename="action.dat", max_num=50):
    ...
    #step 2, 获得所有用户的推荐,每个用户最多推荐50个视频
